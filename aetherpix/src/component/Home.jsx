...
const THEMES = {
  dark: {
    mode: "dark",
    pageBg:
      "linear-gradient(155deg,#02030a 0%,#050a1f 40%,#070512 70%,#02030a 100%)",
  ...
  light: {
    mode: "light",
    pageBg:
      "linear-gradient(155deg,#f4f8ff 0%,#e8eefc 40%,#eef1fb 70%,#f6f9ff 100%)",
...
async function searchGoogleImages(query, apiKey, cx, count = 12) {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      body?.error?.message || `Google search failed (${res.status})`,
    );
  }
...
async function searchImages(query, { apiKey, cx, pageSize = 12 } = {}) {
  if (apiKey && cx) {
    try {
      ...
      console.warn(
        "Google Custom Search failed, falling back to Openverse:",
        e.message,
      );
    }
  }
...
function AnimatedText({ text, style = {}, as: Tag = "span", delay = 0 }) {
  ...
  return (
    <Tag
      ref={ref}
      style={{ display: "inline-block", perspective: 700, ...style }}
    >
        ...
        <span
          ...
          onMouseEnter={(e) => {
            ...
            e.currentTarget.style.transform =
              "translateZ(14px) rotateX(-12deg) rotateY(10deg) scale(1.15)";
          }}
          onMouseLeave={(e) => {
            e.currentTarget.style.transition = "transform 0.25s ease";
            e.currentTarget.style.transform =
    ...
    >
      <span
        style={{
          ...
          opacity: entered ? 1 : 0,
          transition:
            "transform 0.9s cubic-bezier(.16,.84,.44,1), opacity 0.6s ease",
...
function ThreeCanvas() {
  ...
  useEffect(() => {
    ...
    const project = (x, y, z, fov, cx, cy) => {
      ...
      return { x: x * scale + cx, y: y * scale + cy, scale };
    };
    ...
    const draw = (t) => {
      ...
      cubes.forEach((cube) => {
        cube.rx += cube.drx + my * 0.0002;
        cube.ry += cube.dry + mx * 0.0003;
        cube.rz += cube.drz;
        ...
        cube.edges.forEach(([a, b]) => {
          ...
          ctx.strokeStyle = `hsla(${cube.hue},100%,${isLight ? 38 : 70}%,${cube.alpha * avg * (isLight ? 0.5 : 0.8)})`;
          ctx.lineWidth = 1.2 * avg;
          ctx.stroke();
        });
      });
      particles.forEach((p) => {
        ...
        ctx.beginPath();
        ctx.arc(pp.x, pp.y, p.r * pp.scale, 0, Math.PI * 2);
        ctx.fillStyle = `hsla(${p.hue},100%,${isLight ? 40 : 72}%,${(isLight ? 0.22 : 0.35) * pp.scale * tw})`;
        ctx.fill();
      });
    ...
    >
      <span
        style={{
          ...
          borderRadius: "50%",
          background: isDark
            ? "linear-gradient(135deg,#1e90ff,#0b3d91)"
            : "linear-gradient(135deg,#ffd166,#fcb045)",
          display: "flex",
          alignItems: "center",
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        ...
        <button
          ...
          onMouseEnter={(e) => {
            e.target.style.transform = "scale(1.06) translateY(-2px)";
            e.target.style.boxShadow = `0 0 50px ${theme.glow}`;
          }}
          onMouseLeave={(e) => {
            e.target.style.transform = "scale(1) translateY(0)";
            e.target.style.boxShadow = `0 0 32px ${theme.glow}`;
          }}
        >
          Try Free
        </button>
    ...
    <div style={{ width: "100%", maxWidth: 640, marginBottom: 18 }}>
      <button
        ...
        style={{
          ...
          border: "none",
          cursor: "pointer",
          color: usingGoogle ? theme.accent : theme.textFaint,
          fontSize: 12,
          fontWeight: 600,
          display: "flex",
          alignItems: "center",
      ...
      >
        <span
          style={{
            ...
            height: 6,
            borderRadius: "50%",
            background: usingGoogle ? theme.accent : theme.textFaint,
            boxShadow: usingGoogle ? `0 0 8px ${theme.accent}` : "none",
          }}
        />
        {usingGoogle
          ? "Using Google Custom Search API"
          : "Using free Openverse fallback"}{" "}
        · {open ? "hide" : "configure Google API"}
      </button>
      {open && (
        <div
          style={{
        ...
        >
          <p style={{ fontSize: 12, color: theme.textFaint, lineHeight: 1.6 }}>
            Plug in a Google Custom Search JSON API key + Search Engine ID (with
            Image Search enabled) for real Google image results. Get them from
            the Google Cloud Console and Programmable Search Engine. Without
            these, AetherPix automatically uses the free Openverse open-image
            index.
          </p>
          <input
            type="text"
      ...
      {images.map((img, i) => (
        <a
          ...
          style={{
            ...
            transform:
              hovered === img.id ? "scale(1.04) translateY(-2px)" : "scale(1)",
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              justifyContent: "center",
              cursor: "pointer",
              color: theme.mode === "dark" ? "#fff" : "#0a1430",
              fontSize: 12,
          ...
          <div
            style={{
              ...
              opacity: hovered === img.id ? 1 : 0,
              transform:
                hovered === img.id ? "translateY(0)" : "translateY(6px)",
              transition: "all 0.25s ease",
            }}
          >
            <p
...
function Hero() {
  ...
  useEffect(() => {
    const tick = setInterval(
      () => {
        ...
        if (forward.current) {
          charIdx.current++;
          if (charIdx.current >= ph.length) forward.current = false;
        } else {
          charIdx.current--;
  ...
  const pushHistory = (term) => {
    if (!term) return;
    setHistory((h) =>
      [term, ...h.filter((x) => x.toLowerCase() !== term.toLowerCase())].slice(
        0,
        5,
      ),
    );
  ...
  const runSearch = useCallback(
    async (q, keyOverride, cxOverride) => {
      ...
      try {
        ...
        setResultLabel(
          q.trim()
            ? `RESULTS FOR "${q.trim().toUpperCase()}"`
            : "TRENDING SEARCHES",
        );
  ...
  return (
    ...
    >
      ...
      >
        <AnimatedText
          text="Search the infinite universe of images"
          delay={0.3}
        />
      </p>
      <p
      ...
      >
        <AnimatedText
          text="Real images · live search · download anything you find"
          delay={0.6}
        />
      </p>
      ...
      <form
        ...
        style={{ width: "100%", maxWidth: 640, marginBottom: 14 }}
      >
        <Card3D intensity={6} style={{ borderRadius: 50 }}>
          <div
            style={{
              ...
              boxShadow: isFocused
                ? `0 0 48px ${theme.glow}`
                : `0 8px 40px ${theme.glow}`,
              transition: "border-color 0.3s, box-shadow 0.3s",
            }}
          >
            <span
              style={{ paddingLeft: 26, color: theme.textFaint, fontSize: 18 }}
            >
              🔍
            </span>
            <input
      ...
      >
        ...
        >
          ...
          <span style={{ color: theme.textFaint, fontSize: 11 }}>
            {loading
              ? "searching…"
              : `${images.length} results${elapsed ? ` · ${elapsed}s` : ""}`}
          </span>
        </div>
        <ImageGrid images={images} loading={loading} error={error} />
        <p style={{ color: theme.textFaint, fontSize: 11, marginTop: 14 }}>
          Hover an image and tap ⬇ to download it · tap 🎲 to surprise yourself.
        </p>
      </div>
      ...
      >
        ...
        <a
          ...
          onMouseEnter={(e) => {
            e.currentTarget.style.background = `${theme.accent}19`;
...
const FEATURES = [
  {
    ...
    desc: "Advanced ML identifies objects, scenes, and styles in milliseconds.",
  },
  {
    icon: "⚡",
    title: "Lightning Fast",
    desc: "Search billions of images with real-time filters and instant results.",
  },
  {
    icon: "🎯",
    title: "Smart Filters",
    desc: "Filter by color, style, resolution, and custom AI-trained categories.",
  },
  {
    icon: "⬇",
    title: "One-Tap Download",
    desc: "Save any image straight to your device in its original quality.",
  },
  {
    icon: "🌐",
    title: "Google-Powered Search",
    desc: "Plug in your own Google Custom Search API key for true Google image results.",
  },
  {
    icon: "✨",
    title: "Curated Picks",
    desc: "Hand-picked galleries from top photographers and designers globally.",
  },
];
        ...
        {FEATURES.map((f, i) => (
          <Card3D key={i} intensity={12} style={{ borderRadius: 20 }}>
            ...
            >
              ...
              <p
                style={{ color: theme.textDim, lineHeight: 1.75, fontSize: 14 }}
              >
                {f.desc}
              </p>
            </div>
        ...
        {STATS.map((s, i) => (
          <Card3D key={i} intensity={14} style={{ borderRadius: 20 }}>
            ...
            >
              ...
              <p
                style={{ color: theme.textDim, fontSize: 15, fontWeight: 500 }}
              >
                {s.label}
              </p>
            </div>
  ...
  return (
    ...
    >
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        ...
        >
          No hidden fees. Cancel anytime. Start free and scale when you're
          ready.
        </p>
        ...
        >
          <span
            style={{
              fontSize: 13,
              color: !annual ? theme.accent : theme.textFaint,
              fontWeight: 600,
              cursor: "pointer",
              padding: "4px 14px",
              borderRadius: 50,
              background: !annual ? `${theme.accent}1f` : "transparent",
            ...
            <Card3D key={i} intensity={11} style={{ borderRadius: 24 }}>
              ...
              >
                ...
                >
                  ...
                  <span style={{ color: theme.textFaint, fontSize: 14 }}>
                    /{plan.period}
                  </span>
                </div>
                ...
                <button
                  ...
                  onMouseEnter={(e) => {
                    ...
                    e.currentTarget.style.boxShadow = `0 0 52px ${theme.glow}`;
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.transform = "scale(1)";
...
const TESTIMONIALS = [
  {
    text: "AetherPix completely changed how I source images for my clients. The AI filters are insanely precise.",
    name: "Mia Tanaka",
    role: "Creative Director",
  },
  {
    text: "The speed is unreal. I went from spending hours to finding the perfect shot in seconds.",
    name: "Arjun Mehta",
    role: "Photojournalist",
  },
  {
    text: "Finally a search engine that understands visual context, not just keywords.",
    name: "Sofia Reyes",
    role: "UX Designer",
  },
];
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            Join millions of creators discovering the perfect image — no credit
            card, instant access.
          </p>
          <div
            style={{
          ...
          >
            <button
              ...
              onMouseEnter={(e) => {
                e.target.style.transform = "scale(1.06) translateY(-2px)";
                e.target.style.boxShadow = `0 0 70px ${theme.glow}`;
              }}
              onMouseLeave={(e) => {
                e.target.style.transform = "scale(1)";
                e.target.style.boxShadow = `0 0 48px ${theme.glow}`;
              }}
            >
              Try Free
            </button>
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        >
          © 2025 AetherPix. All rights reserved. · Crafted with ✨ · Google
          Custom Search API (optional) or Openverse (CC-licensed open images)
        </div>
      </div>
    </footer>
  );
